import { RAYGUN_REQ_FIELDS } from '../src/constants';
...
describe('helpers', () => {
  describe('handleError', () => {
    ...
    it('defaults prefix to "RaygunSourceMapPlugin"', () => {
      const result = helpers.handleError(new Error('required field missing'));
      expect(result.length).toEqual(1);
      expect(result[0].message).toBe(
        'RaygunSourceMapPlugin: required field missing'
      );
    });
  ...
  describe('validateOptions', () => {
    ...
    it('returns an error if appId is not supplied', () => {
      const options = {
        accessToken: 'aaabbbccc000111',
        publicPath: 'https://my.cdn.net/assets'
      };
      ...
      expect(result.length).toBe(1);
      expect(result[0]).toBeInstanceOf(Error);
      expect(result[0].message).toBe("required field, 'appId', is missing.");
    });
    ...
    it('handles multiple missing required options', () => {
      ...
      const result = helpers.validateOptions(options);
      expect(result).toBeInstanceOf(Array);
      expect(result.length).toBe(RAYGUN_REQ_FIELDS.length);
    });
